...
def build_config(
            ...
            "met_filters": EraModifier(
                {
                    "2016": [
                        ...
                        # "Flag_BadPFMuonDzFilter", # only since nanoAODv9 available. So implement ???
                        "Flag_eeBadScFilter",
                    ],
                    "2017": [
                        "Flag_goodVertices",
    ...
    configuration.add_config_parameters(
        ["et", "mt", "tt"],
        {
            ...
            "tau_sf_file": EraModifier(
                {
                    ...
                    "2018": "data/jsonpog-integration/POG/TAU/2018_UL/tau.json.gz", # emb exists, use it ???
                }
            ),
            "tau_ES_json_name": "tau_energy_scale",
            "tau_id_algorithm": "DeepTau2017v2p1", # we want newer version? 2.5? ???
            ...
            "tau_mufake_es": "nom",
            # boosted taus, not relevant in sm_bbtautau yet!
    ...
    # fatjet base selection (not relevant in sm_bbtautau yet <=> only resolved means no boosted taus):
    configuration.add_config_parameters(
        "global",
        {
            "min_fatjet_pt": 250,
    ...
    # gen b pair for NMSSM analysis () , change into b pair for sm_bbtautau! ???
    configuration.add_config_parameters(
        scopes,
        {
    ...
    configuration.add_config_parameters(
        ["mt", "tt", "et"],
        {
            "vsjet_tau_id": [
                {
                    "tau_id_discriminator": "DeepTau2017v2p1VSjet", # update version to 2.5? ???
    ...
    configuration.add_config_parameters(
        ["tt"],
        {
            "fatjet_trigger_sf_file": EraModifier( # no fatjets in bbtautau... ???
                {
                    "2016preVFP": "",
                    "2016postVFP": "",
                    "2017": "",
    ...
    configuration.add_producers(
        ...
        [
            ...
            # scalefactors.MuonIDIso_SF,                    # no SFs? ???
    ...
    configuration.add_producers(
        ...
        [
            ...
            # scalefactors.Tau_2_oldIsoTauID_lt_SF, this and the next two remove!!!
    ...
    #     ),
    # )
    configuration.add_modification_rule( # modify for SM bbtautau? ???
        ["et", "mt", "tt"],
        AppendProducer(
            producers=[
    ...
    # configuration.add_modification_rule( # not needed for bbtautau
    #     scopes,
    #     RemoveProducer(
    #         producers=[
    ...
    if sample in ["nmssm_Ybb", "nmssm_Ytautau"]: # change into SM bbtautau? ???
        configuration.add_outputs(
            scopes,
            [
                q.gen_b_pt_1,
    ...
    #########################
    # particleNet Xbb scale factor uncertainties # not relevant for bbtautau? ???
    #########################
    # configuration.add_shift(
    #     SystematicShift(
    #         name="pNetXbbSFUp",
