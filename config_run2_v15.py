...
def build_config(
    ...
    if int(era[:4]) < 2022: # ToDo: ???
        default_met_filters.extend(["Flag_HBHENoiseFilter", "Flag_HBHENoiseIsoFilter"])
        if sample in ["embedding", "embedding_mc"]:
            default_met_filters.remove("Flag_BadPFMuonDzFilter") # not available in nanoAOD of embedding
    if int(era[:4]) >= 2017:
    ...
    configuration.add_config_parameters(
        scopes,
        {
            ...
            "btag_sf_file": EraModifier( # TODO Update run2 when btag efficiency is measured (needed for new SFs producers)
                {
                    "2016preVFP": "/cvmfs/cms-griddata.cern.ch/cat/metadata/BTV/Run2-2016preVFP-UL-NanoAODv15/2026-06-18/btagging.json.gz",
                    "2016postVFP": "/cvmfs/cms-griddata.cern.ch/cat/metadata/BTV/Run2-2016postVFP-UL-NanoAODv15/2026-06-18/btagging.json.gz",
                    "2017": "/cvmfs/cms-griddata.cern.ch/cat/metadata/BTV/Run2-2017-UL-NanoAODv15/2026-06-18/btagging.json.gz",
                    "2018": "/cvmfs/cms-griddata.cern.ch/cat/metadata/BTV/Run2-2018-UL-NanoAODv15/2026-06-18/btagging.json.gz",
    ...
    configuration.add_config_parameters(
        ["mt", "tt", "et"],
        {
            ...
            "tau_vsjet_wp": "Loose", ##change again to Loose if it becomes available
    ...
    configuration.add_config_parameters(
        ["et", "ee", "em"],
        {
            ...
            "ele_id_sf_name": "Electron-ID-SF" if int(era[:4]) >= 2022 else "UL-Electron-ID-SF",
                    ...
                    "vsmu_tau_id": [
                        {
                            ...
                            "vsmu_tau_id_WP": "{wp}".format(wp=wp),
                            "vsele_tau_id_WP": "Missing or non existent",
                            "vsjet_tau_id_WP": "Missing or non existent", 
    ...
    configuration.add_producers(
        ...
        [
            ...
            jets.JetBTagUParT,
            jets.JetRho,
            # jets.JetID, # ToDo: json files in progress, do pseudo code instead !!!
            jets.JetID_pseudo,
            jets.JetVetoMapVeto,
    ...
    if int(era[:4]) < 2022: # Not necessarily: for v15 Run-2(UL) this should nnot be used...
        if not run2_v15:
            configuration.add_modification_rule(
                "global",
                ReplaceProducer(
    ...
    if 2022 <= int(era[:4]) < 2024:
        ...
        configuration.add_modification_rule(
            "global",
            ReplaceProducer(
                producers=[met.MetBasics_v15, met.MetBasics],
                exclude_samples=["fake_era"],
            ),
        )
        # configuration.add_modification_rule( ### # TODO Update run2 to 06-05 when btag efficiency is measured (needed for new SFs producers)
        #     scopes,
        #     ReplaceProducer(
    ...
    elif int(era[:4]) >= 2024 or run2_v15:
        configuration.add_outputs(
            "global",
            [
    ...
    # Measure btag efficiency from MC:
    ...
    elif int(era[:4]) >= 2022 or run2_v15:
        with defaults(
            scopes="global",
            shift_key="ele_es_variation",
    ...
    configuration.add_shift(
        SystematicShift(
            ...
            shift_config={
                ...
                ("em"): {"muon_id_sf_variation": "systup"},
            },
            producers={
                ("mt"): [
                    scalefactors.MuonIDIso_SF,
    ...
    configuration.add_shift(
        SystematicShift(
            ...
            shift_config={
                ...
                ("em"): {"muon_id_sf_variation": "systdown"},
            },
            producers={
                ("mt"): [
                    scalefactors.MuonIDIso_SF,
    ...
    configuration.add_shift(
        SystematicShift(
            ...
            shift_config={
                ...
                ("em"): {"muon_iso_sf_variation": "syst_up"},
            },
            producers={
                ("mt"): [
                    scalefactors.MuonIDIso_SF,
    ...
    configuration.add_shift(
        SystematicShift(
            ...
            shift_config={
                ...
                ("em"): {"muon_iso_sf_variation": "syst_down"},
            },
            producers={
                ("mt"): [
                    scalefactors.MuonIDIso_SF,
    ...
    if int(era[:4]) >= 2022:
        add_shift(
            ...
            shift_map={"Up":"up", "Down":"down"},
            samples=["dyjets_powheg", "dyjets_amcatnlo", "dyjets_amcatnlo_ll",
                     "dyjets_amcatnlo_tt", "electroweak_boson"],
        )
