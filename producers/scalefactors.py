...
with defaults(scopes=["mt", "mm"], input=[q.pt_1, q.eta_1]):
    Muon_1_ID_SF = Producer(
        call='''physicsobject::muon::scalefactor::Id({df}, correctionManager, {output}, {input}, "{muon_sf_file}", "{muon_id_sf_name}", "{muon_id_variation}")''',
        output=[q.id_wgt_mu_1],
    )
    Muon_1_Iso_SF = Producer(
        call='''physicsobject::muon::scalefactor::Iso({df}, correctionManager, {output}, {input}, "{muon_sf_file}", "{muon_iso_sf_name}", "{muon_iso_variation}")''',
