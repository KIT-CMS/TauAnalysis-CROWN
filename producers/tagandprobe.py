...
with defaults(scopes=["mm"]):
    ...
    with defaults(output="flagname_1"):
        with defaults(input=[q.p4_1] + TrigObj_collection):
            MuMuSingleMuonTriggerFlags_1 = ExtendedVectorProducer(
                call='trigger::tagandprobe::SingleObjectFlag({df}, {output}, {input}, "{hlt_path}", {ptcut}, {etacut}, {trigger_particle_id}, {vec_open}{filterbit}{vec_close}, {max_deltaR_triggermatch}, {triggerobject_ptcut} )',
                vec_config="singlemuon_trigger",
            )
            MuMuSingleMuonTriggerBitFlags_1 = ExtendedVectorProducer(
                call="trigger::tagandprobe::MatchSingleTrigObj({df}, {output}, {input}, {ptcut}, {etacut}, {trigger_particle_id}, {vec_open}{filterbit}{vec_close}, {max_deltaR_triggermatch}, {triggerobject_ptcut} )",
        ...
        with defaults(input=[q.p4_1, q.p4_2] + TrigObj_collection):
            MuMuDoubleMuonTriggerFlags_1 = ExtendedVectorProducer(
                call='trigger::tagandprobe::DoubleObjectFlag({df}, {output}, {input}, "{hlt_path}", {p1_ptcut}, {p2_ptcut}, {p1_etacut}, {p2_etacut}, {p1_trigger_particle_id}, {p2_trigger_particle_id}, {vec_open}{p1_filterbit}{vec_close}, {vec_open}{p2_filterbit}{vec_close}, {max_deltaR_triggermatch}, {p1_triggerobject_ptcut}, {p2_triggerobject_ptcut} )',
    ...
    with defaults(output="flagname_2"):
        ...
        with defaults(input=[q.p4_2, q.p4_1] + TrigObj_collection):
...
with defaults(scopes=["ee"]):
    ...
    with defaults(vec_configs="singleelectron_trigger"):
        ...
        ElElSingleElectronTriggerFlags_2 = ExtendedVectorProducer(
            call='trigger::tagandprobe::SingleObjectFlag({df}, {output}, {input}, "{hlt_path}", {ptcut}, {etacut}, {trigger_particle_id}, {vec_open}{filterbit}{vec_close}, {max_deltaR_triggermatch}, {triggerobject_ptcut} )',
            input=[q.p4_2] + TrigObj_collection,
            output="flagname_2",
        )
    ...
    with defaults(call="event::quantity::Get<bool>({df}, {output}, {input}, 1)"):
        ElectronID_WP90_2 = Producer(input=[nanoAOD.Electron_mvaNoIso_WP90, q.dileptonpair], output=[tp_q.id_wp90_2])
        ElectronID_WP80_2 = Producer(input=[nanoAOD.Electron_mvaNoIso_WP80, q.dileptonpair], output=[tp_q.id_wp80_2])
